...
2. **Frontend: Prettier**를 사용하여 코드 일관성을 유지하였습니다.
3. **CD With AWS**
    1. Github Actions, Docker Hub, AWS 서비스를 사용하여 **CI Pipeline을 구축**하였습니다.
    2. 레포지토리가 릴리즈 될 경우 도커 허브에 이미지를 자동으로 Push합니다.
    3. Route53, S3, EC2, ALB, Target Group, AWS Certificate Manager 등의 서비스를 사용하여 HTTPS로 서빙되었습니다.

### 💰 비용 최적화(Cost Optimization)

1. **AWS Spot Instance**
    1. 초기 서버 비용의 약 65%를 절감하였습니다.
2. CloudFlare의 R2를 도입하여 정적 파일 스토리지의 비용을 절감하려 하였으나, 예상보다 낮은 R2 버킷의 완성도 때문에 채택하지 않았습니다.

<br />

## 4.2.백엔드 개발(Backend Development)
...
## 4.3. 프론트엔드 개발(Frontend Development): Next.js
...
1. 🎨 **컴포넌트 재사용성(Class Variance Authority)**
...
1. **Milvus 클라이언트를 통한 유사도 검색**
   ...
   4. 검색어 또는 사용자의 히스토리를 임베딩으로 변환하여 Milvus 데이터베이스에서 유사도를 검색하고, DB에서 정보를 반환하는 구조입니다.
...
- 상품 명, 가격, 이미지 및 판매 링크(외부 URL)
- 등록된 상품 목록 및 상세 조회
    - 무한 스크롤로 이전 글 읽기
- 작성자 본인에 한해 등록한 상품 글 수정 및 삭제

**🍊 게시글**

- 제목, 내용, 이미지, 태그, 조회수 및 좋아요 수 확인
- 홈피드에서 팔로우한 사용자의 게시글 조회
- 작성자 본인에 한해 등록한 게시글 수정 및 삭제
- 개인화 피드 페이지네이션 리스트, 검색 피드 페이지네이션 리스트 제공
...
어느때보다 정신없이 지나간 3주가 아닌가 싶습니다. 무언가 부족한 것이 없었는지 이제야 아쉬움이 느껴집니다. 
